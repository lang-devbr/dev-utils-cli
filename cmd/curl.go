...
var (
	curlCommand = &cobra.Command{
		Use:   "curl",
		Short: "Command to build curl",
		Long:  "Command to build curl with parameters",
		RunE:  curlExecute,
	}
)

func init() {
	rootCmd.AddCommand(curlCommand)
}

func curlExecute(cmd *cobra.Command, args []string) error {
	r, err := tea.NewProgram(initModel()).Run()
	if err != nil {
		return err
	}
	if m := r.(model); m.ok {
